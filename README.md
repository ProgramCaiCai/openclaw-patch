...
Patch bundle for branch `fix/compact-content-normalize`, regenerated from:

```bash
git format-patch main --output-directory /tmp/openclaw-patches-v2/
```

## This Refresh

- Added provider-tool wrapping support in `ctx_safe_call` (`read`/`exec`/`process`).
- Unified `ctx_safe_call` naming across agent implementation and tests.
- Fixed Telegram native command routing to use direct `ChatType`.
- Hardened `ctx_safe_call` provider resolution and signal propagation after review.
...
- 本次为 `fix/compact-content-normalize` 分支的完整补丁刷新。
- 重点新增：`ctx_safe_call` provider tools 支持、命名统一、`ChatType` 路由修复、review 问题修复。
- 基线 commit：`1d46ca3a95c7ff2669cc9c2a231fc460a2a3cbbb`，补丁数量：`44`。
